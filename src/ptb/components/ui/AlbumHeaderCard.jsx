...
export default function AlbumHeaderCard({ isReadonly }) {
  return (
    <header className={cn(ptb.card, "shrink-0 px-3 py-2.5 sm:px-5 sm:py-3")}>
      {isReadonly ? (
        <p className={cn(ptb.textBody, "text-[13px]")}>
          Album chỉ xem — chuyến thuê đã kết thúc.
        </p>
      ) : (
        <p className="flex items-center gap-2 text-[13px] font-semibold text-emerald-800 sm:text-[14px]">
